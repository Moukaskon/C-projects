...
void PrintListOfAllUsers(HashListType Hlist);
void FindRole(int code, char *role);
...
int main(){
    ...
    while(TRUE){
        ...
        scanf("%d%c", &AnItem.RecKey, &bin);
        SearchHashList(Alist, AnItem.RecKey, &Loc, &Pred);
        if(Loc != -1){
            FindRole(Alist.List[Loc].Data.role, role);
            printf("[%d,%s,%s]\n", Alist.List[Loc].RecKey, Alist.List[Loc].Data.name, role);
        }else{
            printf("There is no user with code %d", AnItem.RecKey);
        }
        printf("\nContinue Y/N: ");
        scanf("%c%c", &answer, &bin);
        if(answer == 'N' || answer == 'n'){
...
void PrintListOfAllUsers(HashListType Hlist){
    ...
    for(i = 0; i < HMax; i++){
        ptr = Hlist.HashTable[i];
        if(ptr != EndOfList){
            printf("Synonyms, collision at position : %d\n", i);
        }
        while(ptr != EndOfList){
            FindRole(Hlist.List[ptr].Data.role, role);
            printf("%d: [%d,%s,%s]\n", ptr, Hlist.List[ptr].RecKey, Hlist.List[ptr].Data.name, role);
            ptr = Hlist.List[ptr].Link;
        }
    }
...
void FindRole(int code, char *role){
    char role1[] = {"Student"};
    char role2[] = {"postgraduate student"};
    char role3[] = {"teacher"};
    char role4[] = {"visitor"};
