...
int main(){
    BinTreePointer Root, Item1, Item2, Loc, Loc2;
    BinTreeElementType Insertion;
    int i, noel;
    char bin;
    boolean found1, found2;

    CreateBST(&Root);
    Item1 = (BinTreePointer)malloc(sizeof (struct BinTreeNode));
    Item2 = (BinTreePointer)malloc(sizeof (struct BinTreeNode));
    printf("Enter number of elements for the tree: ");
    scanf("%d%c", &noel, &bin);

    for(i = 0; i < noel; i++){
        printf("Enter number: ");
        scanf("%d%c", &Insertion, &bin);
        RecBSTInsert(&Root, Insertion);
    }
    ...
    while(TRUE){
        ...
        scanf("%d%c", &Item1 ->Data, &bin);
        printf("Give second element of BST: ");
        scanf("%d%c", &Item2 ->Data, &bin);
        RecBSTSearch(Root, Item1 ->Data, &found1, &Loc);
        RecBSTSearch(Root, Item2 ->Data, &found2, &Loc2);
        if(found1 && found2){
            break;
        }
        printf("Den uparxoun ta stoixeia. Try again\n");
    }
    Loc = LCANode(Root, Item1, Item2);
    printf("LCA Node of %d and %d is Node with element %d", Item1 ->Data, Item2 ->Data, Loc ->Data);
...
BinTreePointer LCANode(BinTreePointer Root, BinTreePointer Node1, BinTreePointer Node2){
    ...
    if(Node1 ->Data > Root ->Data && Node2 ->Data > Root ->Data)
    {
        if(Root ->RChild ->Data == Node1 ->Data || Root ->RChild ->Data == Node2 ->Data){
            return Root;
        }
       Loc = LCANode(Root ->RChild, Node1, Node2);
    }
    if(Node1 ->Data < Root ->Data && Node2 ->Data < Root ->Data)
    {
        if(Root ->LChild ->Data == Node1 ->Data || Root ->LChild ->Data == Node2 ->Data){
            return Root;
        }
        Loc = LCANode(Root ->LChild, Node1, Node2);
    }
return Loc;
}
